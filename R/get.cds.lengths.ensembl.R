...
gene.list <- unique(bm.filt.sort.merge$ensembl_gene_id)
gene.genomic_coding.len <- data.frame(gene_name = gene.list)
gene.genomic_coding.len$len <- do.call('c', lapply(gene.list, function(x) sum(bm.filt.sort.merge$genomic_coding_len[bm.filt.sort.merge$ensembl_gene_id == x])))
head(gene.genomic_coding.len)

# Write to file
write.table(gene.genomic_coding.len, "bm.exon.union.length.ensembl.txt", quote = FALSE, row.names = FALSE, col.names = TRUE, sep = '\t')
