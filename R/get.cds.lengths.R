...
gene.list <- unique(bm.filt.sort.merge$external_gene_name)
gene.genomic_coding.len <- data.frame(gene_name = gene.list)
gene.genomic_coding.len$len <- do.call('c', lapply(gene.list, function(x) sum(bm.filt.sort.merge$genomic_coding_len[bm.filt.sort.merge$external_gene_name == x])))
